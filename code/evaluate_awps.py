"""
Read in problems from Algebraic-Word-Problem-Solver (AWPS) dataset and evaluate a model on them.
Assumes that the Algebraic-Word-Problem-Solver github repo
    (https://github.com/wangxr14/Algebraic-Word-Problem-Solver/tree/master)
    is cloned in a sister directory to this repository.
"""
from constants import *
from evaluation_utils import *

AWPS_PATH = os.path.abspath(os.path.join('..','..','Algebraic-Word-Problem-Solver','data'))
AWPS_RESULTS_PATH = os.path.join(RESULTS_PATH,'awps')

VERBOSE= True

def run_AWPS (model:PromptModel, skip:int = 0, limit:int = 1, scorer:Callable[[str, str],bool] = number_scorer,
               taskset:str = 'test', skip_condition:Callable[[str],bool] = None,
               batch:bool = True, questions:list = [], answers:list = []):
    """
    Load AWPS dataset and evaluate the model over it.
    ...
    - taskset: the task name within AWPS
    ...
    built_in = ['AddSub','MultiArith','SingleOp'] # tasksets which are built into AWPS

    # if questions kwarg is not provided, read in from task file
    if len(questions) < 1:
        inputs = []
        if taskset in built_in: # built-in tasks from AWPS are stored in jsonl files
            task_path = os.path.join(AWPS_PATH, taskset+'.json')
            with open(task_path, 'r', encoding='utf-8') as json_file:
                inputs = json.load(json_file)
            questions = [i["sQuestion"] for i in inputs]
            answers = [i['lSolutions'][0] # all questions have a single correct answer.
                       for i in inputs]
    ...
    if batch: # batched mode
        ...
        results_log, response_log, end, missed_questions = evaluate_batch(questions, answers, model=model,
                                                                          scorer=scorer, taskset=taskset, task='awps')
    else: # non-batched mode
        for q, a in question_iter:
            try:
                score, response = evaluate_question(q, a, model=model, scorer=scorer, taskset=taskset, task='awps')
...
if __name__ == "__main__": # run evaluation
    ...
    parser.add_argument("--taskset", type=str, default=DEFAULT_TASKSETS['awps'],
                        help="Task set to be evaluated (i.e. MultiArith).")
    ...
    results_path = os.path.abspath(os.path.join(AWPS_RESULTS_PATH,TASKSET+'.csv'))
    summary_path = os.path.abspath(os.path.join(AWPS_RESULTS_PATH, 'results_summary.csv'))
    ...
    if EVALUATE == 'new': # query the model for new results
        ...
        try:
            ...
            for SKIP in range(SKIP, skip_limit, skip_interval): # temporary thing to help slow batching
                start = time.asctime(time.localtime())
                if VERBOSE:
                    print(f'Evaluating AWPS {TASKSET} on model: {model.name+" "+model.style}. Start time: {start}')
                task_result_dict = {'Start_Time':start}
                results, errors = run_AWPS(model, limit=skip_interval, skip=SKIP, taskset=TASKSET,
            ...
            while len(error_results)>0 and rerun<MAX_RERUN:
                ...
                start = time.asctime(time.localtime())
                if VERBOSE:
                    print(f'Re-evaluating AWPS {TASKSET} on model: {model.name} for the {rerun}th time. Start time: {start}')
                task_result_dict = {'Start_Time':start}
                results, repeat_errors = run_AWPS(model, limit=-1, taskset=TASKSET, questions= questions,
