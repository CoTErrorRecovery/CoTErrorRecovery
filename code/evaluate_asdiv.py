"""
Read in problems from ASDiv dataset and evaluate a model on them.
Assumes that the ASDiv github repo
    (github.com/chaochun/nlu-asdiv-dataset/tree/master)
...
ASDIV_PATH = os.path.abspath(os.path.join('..','..','nlu-asdiv-dataset','dataset'))
ASDIV_RESULTS_PATH = os.path.join(RESULTS_PATH,'asdiv')

VERBOSE= True

# def is_numeric (s:str) -> bool:
#     """
#     Check if a string is numeric (any form which can be evaluated as a number).
...
def run_ASDiv (model:PromptModel, skip:int = 0, limit:int = 1, scorer:Callable[[str, str],bool] = number_scorer,
               taskset:str = 'ASDiv', skip_condition:Callable[[str],bool] = None, batch:bool = True,
               questions:list = [], answers:list = []):
    """
    Load ASDiv dataset and evaluate the model over it.
    ...
    - taskset: the task name within ASDiv (important for running perturbations and other custom tasks)
    ...
    built_in = ['ASDiv'] # tasksets which are built into ASDiv

    # if questions kwarg is not provided, read in from task file
    if len(questions) < 1:
        inputs = []
        if taskset in built_in: # built-in tasks from ASDiv are stored in xml files
            task_path = os.path.join(ASDIV_PATH, taskset+'.xml')
            inputs = ET.parse(task_path).getroot()[0].findall('Problem')
            inputs = [i for i in inputs
                      if i.find("Answer").text.split()[0].isnumeric()
                    #   if is_numeric(i.find("Answer").text.split()[0])
                      ] # ignore questions with non-integer answers (leaves 2097 valid questions in ASDiv)
            questions = [i.find("Body").text +' '+ i.find("Question").text for i in inputs]
            answers = [i.find("Answer").text.split()[0] for i in inputs]
        else: # custom tasks (from perturbations) are stored in json files
            task_path = os.path.abspath(os.path.join(ASDIV_RESULTS_PATH,
    ...
    if batch: # batched mode
        ...
        results_log, response_log, end, missed_questions = evaluate_batch(questions, answers, model=model,
                                                                          scorer=scorer, taskset=taskset, task='asdiv')
    else: # non-batched mode
        for q, a in question_iter:
            try:
                score, response = evaluate_question(q, a, model=model, scorer=scorer, taskset=taskset, task='asdiv')
...
if __name__ == "__main__": # run evaluation
    ...
    parser.add_argument("--taskset", type=str, default=DEFAULT_TASKSETS['asdiv'],
                        help="Task set to be evaluated (i.e. ASDiv).")
    ...
    results_path = os.path.abspath(os.path.join(ASDIV_RESULTS_PATH,TASKSET+'.csv'))
    summary_path = os.path.abspath(os.path.join(ASDIV_RESULTS_PATH, 'results_summary.csv'))
    ...
    if EVALUATE == 'new': # query the model for new results
        ...
        try:
            ...
            for SKIP in range(SKIP, skip_limit, skip_interval): # temporary thing to help slow batching
                start = time.asctime(time.localtime())
                if VERBOSE:
                    print(f'Evaluating ASDiv {TASKSET} on model: {model.name+" "+model.style}. Start time: {start}')
                task_result_dict = {'Start_Time':start}
                results, errors = run_ASDiv(model, limit=skip_interval, skip=SKIP, taskset=TASKSET,
            ...
            while len(error_results)>0 and rerun<MAX_RERUN:
                ...
                start = time.asctime(time.localtime())
                if VERBOSE:
                    print(f'Re-evaluating ASDiv {TASKSET} on model: {model.name} for the {rerun}th time. Start time: {start}')
                task_result_dict = {'Start_Time':start}
                results, repeat_errors = run_ASDiv(model, limit=-1, taskset=TASKSET, questions= questions,
