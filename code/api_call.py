"""
Functions to process inputs, make calls to the Open AI and Claude 3 APIs, and process outputs from the API.
"""
...
WAIT = 1 # seconds to wait between each call to the api, to avoid rate limit
TIMEOUT = 40 # seconds to wait (per question) before timeout (mostly handled in threads, not in API calls)

# set api keys. You can manually paste them in here or set them as an environmental variable (recommended)
...
TOGETHER_MODELS = ['meta-llama/Llama-3-8b-chat-hf','meta-llama/Llama-3-70b-chat-hf']
...
CHAT_MODELS = ['gpt-4', 'gpt-4-0314', 'gpt-4-0613', 'gpt-4-32k', 'gpt-4-32k-0314', 'gpt-4-32k-0613', 'gpt-3.5-turbo',
               ...
               'gpt-4-1106-preview']
RATE_LIMITS = {'gpt-4': (10000, 300000),
               'claude': (4000, 400000),
               'chat': (10000, 2000000),
               'text': (3000, 250000)} # rate limits for different models, in requests and tokens per minute
...
def query_model(batches:list, api:str, model:str, echo:bool = False, max_length:int = MAX_TOKENS,
    ...
    - api: name of the API to use ('openai' or 'anthropic')
    ...
    if thread_num > 1: # multi-threaded
    ...
    elif (api != 'openai') or (parse_base_model_name(model) in CHAT_MODELS): # single thread - chat model
        for question in batches: # pass questions to model 1 at a time
            ...
            elif api == 'anthropic': # format messages - do not include system prompt
                messages = build_messages(question, split_answer=split_answer, post_reasoning_prompt=post_reasoning_prompt)
            else:
...
f'''response = client.chat.completions.create(
    ...
    messages={messages},
        ...
        for batch in batches:
            ...
            time.sleep(WAIT)
            try:
                response = client.completions.create(prompt=batch,
                model=model,
                temperature = temp,
                max_tokens=max_length,
                best_of = BEST_OF,
...
def generate_initial_answer (prompt:list, echo:bool = False, q_a:bool = True, api:str = 'openai', model:str = MODEL,
    ...
    - api: name of the API to use ('openai' or 'anthropic')
    - model: name of the model in the API
    - max_length: max number of tokens in text completion
    - verbose: whether the process prints to the console
    ...
    if thread_num > 1:
        results = {}
        query_model(batches, api, model, echo, max_length = max_length, thread_num=thread_num, results=results,
        ...
        results = query_model(batches, api, model, echo, max_length = max_length, thread_num=thread_num,
                              post_reasoning_prompt = post_reasoning_prompt, temp=temp, system_prompt=system_prompt)
...
def generate (prompt:[list,str], style:str = '', model:str = MODEL, max_length:int = MAX_TOKENS,
    ...
    if (not style) or ('fewshot' in style or 'direct' in style):
        ...
        reasoning = generate_initial_answer(prompt, echo=False, q_a=False, api=api, model=model,
    ...
    if len(final_prompt) < 1 or all([p is None for p in final_prompt]):
        ...
        final = generate_initial_answer(final_prompt, echo=False, q_a=False, api=api, model=model,
                                        max_length = max_length, verbose=verbose, thread_num=thread_num,
                                        post_reasoning_prompt = post_reasoning_prompt, temp=temp, system_prompt=system_prompt)
...
def get_cond_logprobs (prompt:str, target:str, prior_prompt:str, style:str = "", model:str = MODEL, verbose:bool = True):
    ...
    """
    if (model in ANTHROPIC_MODELS) or (model in CHAT_MODELS):
        raise ValueError("Token logprobabilities are not supported by OpenAI chat models or any Anthropic models.")
